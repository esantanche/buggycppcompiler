...
// Variabili statiche
//----------------------------------------------------------------------------
//extern CLASSIFICA_TRENI * CLASSIFICA_TRENI::Classifiche;  // EMS002 VA
...
void  PROFILER::PutStdOut(BOOL Parziale)   { // Scrive i dati sullo standard output
   ...
   puts("===================================================");
   // EMSb001 tolgo riferimento a stdout che da fastidio alla compilazione sotto
   // Win32s
   // fflush(stdout);
};
...
F_DTM _export GetTimestampDll(const char * NomeDLL){
   ...
   HMODULE   Idll = 0;
   ULONG Len = 256;
   rc = DosLoadModule((PSZ) Msg,sizeof(Msg),(PSZ)(char*)NomeDLL,&Idll); // EMS003 VA (PSZ)
   if(rc){
...
void CLASSIFICA_TRENI::CaricaClassifiche(const STRINGA & PathDati){
   ...
   // EMS004 VA if (Classifiche) return;
   ...
   TRACESTRING("HO CARICATO LE CLASSIFICHE");
};
