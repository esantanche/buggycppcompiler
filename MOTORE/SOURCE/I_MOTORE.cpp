...
CLASSIFICA_TRENI * CLASSIFICA_TRENI::Classifiche;  // EMS004 VA
...
void MM_I_PERCORSO::ImpostaCCRDestinazione(ID IdOrigine, ID IdDestinazione){
   ...
   if (DatiTariffazione.KmConcessi1 == 0) {
      ...
      // EMS005 Win sostituisco SCAN_NUM_WIN a SCAN_NUM
      SCAN_NUM_WIN(Conc,Destinazione.NumeroConcesse,DatiConcessi,STAZ_FS::D_CONCESSE){
...
CacheMotore((PathCacheGrafo == NUSTR) ? PathE : PathCacheGrafo )
{
   ...
   // EMS006 Win
   // Mi serve per avere la certezza che le classifiche vengano caricate
   CLASSIFICA_TRENI::Classifiche = NULL;
   // end EMS006
...
//----------------------------------------------------------------------------
BYTE __WIN_FASTCALL LinguaInUso(){
   #undef TRCRTN
...
MM_PERCORSO * MM_RETE_FS::ViaggioLibero(ARRAY_ID & NodiDelViaggio,BOOL AbilitaCumulativo){
   ...
   if (Perco.SetUtilizzandoPolimetriche(NodiDelViaggio)) {
      TRACESTRING("Prima di Perco.LunghezzaTariffabile();");
      Perco.LunghezzaTariffabile();
      TRACESTRING("Esco da Perco.LunghezzaTariffabile();");
      if(Perco.Istr == NULL){
...
MM_ELENCO_PERCORSO_E_SOLUZIONI * MM_RETE_FS::M300_RichiestaPerRelazione(ID Da,ID A,
   ...
   //TRACESTRING("Sono dentro a M300 I_MOTORE.CPP 1311"); // EMS Win
...
MM_ELENCO_PERCORSO_E_SOLUZIONI * MM_RETE_FS::SIPAX_RichiestaPerRelazione(
   ...
   //TRACESTRING("Sono entrato in SIPAX_RichiestaPerRelazione"); // EMS
   GRAFO & Grafo = GRAFO::Gr();
   ...
   if (!OkOrario){
      ...
      ERRSTRING("Impossibile impostare orario per il giorno "+STRINGA(Data));
      ...
      if(Motivo != PER_INFORMATIVA){
         ...
         if(!OkOrario){
            ERRSTRING("Fallito tentativo di impostare orario per il giorno "+STRINGA(Data2));
...
//<<< if  !OkOrario
   ...
   if (!OkOrario){
      ...
      if(!TrovatoSuCache){
         ...
         BOOL Result;
         TRACESTRING("primadi NewRisolvi: Da="+STRINGA(Da)+" A="+STRINGA(A)+" DATA: "+
             STRINGA(Data.Giorno)+"."+STRINGA(Data.Mese)+"."+STRINGA(Data.Anno)+" "
             +STRINGA(Ora/60)+":"+STRINGA(Ora%60)   );
...
// EMS007 VA spostata qui dalla funzione Sort
struct SUMMARY {
   WORD OrdineBase   ;
   WORD TempoViaggio ;
   BYTE Extra;
...
void MM_ELENCO_PERCORSO_E_SOLUZIONI::Sort(WORD Ora){
   ...
   // EMS007 strutttura SUMMARY spostata fuori
