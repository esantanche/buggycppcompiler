...
BYTE _WIN_FASTCALL PERIODICITA::Circola4(BYTE ParteGiornoPrecedente ){
   #undef TRCRTN
...
ELENCO_S PERIODICITA::Decod(BOOL ConTestata,BOOL DoubleSpace,BOOL Points){
   ...
   // EMS003 VA la stringa di inizializzazione di Vuoto e di Punti erano piu' lunghe del
   // necessario di un carattere
...
//----------------------------------------------------------------------------
BOOL __WIN_FASTCALL T_PERIODICITA::ConfrontaShiftata( const T_PERIODICITA & PeriodShiftata, signed char Shift) const {
   #undef TRCRTN
...
BOOL __WIN_FASTCALL T_PERIODICITA::ConfrontaShiftando( T_PERIODICITA & PeriodDaShiftare, signed char Shift) const {
   #undef TRCRTN
...
//----------------------------------------------------------------------------
void __WIN_FASTCALL T_PERIODICITA::ShiftMe(signed char Shift){
   #undef TRCRTN
...
void PERIODICITA_IN_CHIARO::Popola( PERIODO_PERIODICITA & Origine , BYTE Euristic){
   ...
   PERIODO_PERIODICITA::TIPO_PERIODO Tipo;
   // EMS004 VA il compilatore non accetta l'istruzione int(Tipo)++
   int * piTipo = (int *) &Tipo;  // EMS004 VA
   for ( Tipo = PERIODO_PERIODICITA::TUTTI_I_GIORNI;
...
//       for (GIORNI_SETTIMANA gg = LUNEDI; gg <= DOMENICA; int(gg)++ ) {
         GIORNI_SETTIMANA gg;                    // EMS005
         int * pigg = (int *) &gg;                             // EMS005
         for (gg = LUNEDI; gg <= DOMENICA; *(pigg) ++ ) {    // EMS005
            if ((Giorni & (1 << gg))== 0 ) {
