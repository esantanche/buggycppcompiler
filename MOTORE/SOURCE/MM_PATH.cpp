...
// EMS003 Win tolgo inline
int HEAP<NODO_CAMBIO>::Confronta(NODO_CAMBIO * A, NODO_CAMBIO *B){ return int(A->Costo1) - int(B->Costo1);};
...
COLLEGAMENTO * NODO_CAMBIO::CollegamentoDaId(ID Target){
   ...
   // begin EMS004 Win
   /* precedente corpo della funzione:
   ...
   */
   ...
   Coll = Collegamenti();
   bTrovato = FALSE;
   for (i = 0; i < NumeroCollegamenti; i ++, Coll++ ) {
      if ((Coll->Id) == Target) {
         bTrovato = TRUE;
         break;
      }
...
BOOL NODO_CAMBIO::Risolvi(ID Origine, ID Destinazione){
   ...
   //TRACESTRING("Entro in NODO_CAMBIO::Risolvi");

   #ifdef DBG11
   static FirstTime = TRUE;
   if (FirstTime) {
      // EMS005 Win inserisco gestione della limitazione degli errori
      int iNum_errori = 0;
      TRACESTRING("Test Simmetria collegamenti");
      NODO_CAMBIO * Nodo = NodiDiCambio + 1; // Il primo NON e' valorizzato
      for (int i1 = TotNodiDiCambio-1; i1 > 0 && iNum_errori < 50 ;i1 -- ) {
...
//<<< if  FirstTime
   ...
   ERRSTRING(Msg);
   #endif
   //TRACESTRING("Prima di scan_num");
   // EMS006 Win sostituisco SCAN_NUM_WIN a SCAN_NUM
   SCAN_NUM_WIN(NodiDiCambio,TotNodiDiCambio,Nodo,NODO_CAMBIO){
      Nodo->Costo1     = 0xffff;
      Nodo->Nodo1      = 0 ;
   ...
   //TRACEINT("Destinazione=",Destinazione);
   ...
   //static HEAP2<NODO_CAMBIO> Heap(512);       // EMS007 Win aumento dimensione heap
   static HEAP2<NODO_CAMBIO> Heap(1024);
   ...
   #endif
   // EMS008 Win altra eliminazione di reference forzato in ciclo (cambiato nel puntatore
   // pNodoScanned)
   NODO_CAMBIO * pNodoScanned;
   while (Heap.Dim()) {
      //TRACEINT("prima Heap.Dim()=",Heap.Dim());

      pNodoScanned = Heap.DeleteMin();
      pNodoScanned->Stato = SCANNED;

      //TRACEINT("dopo Heap.Dim()=",Heap.Dim());

      #ifdef DBG3
      //pNodoScanned->TraceId("€€€€€€€ IdNodoScanned Costo = "+STRINGA(pNodoScanned->Costo1)+ " NumeroCollegamenti "+STRINGA(pNodoScanned->NumeroCollegamenti));
      #endif
      ...
      //TRACEPOINTER("Prima del ciclo pNodoScanned->Collegamenti()=",pNodoScanned->Collegamenti());

      // Per tutti i primi vicini del nodo attivo
      // EMS006 Win sostituisco SCAN_NUM_WIN a SCAN_NUM
      SCAN_NUM_WIN(pNodoScanned->Collegamenti(),pNodoScanned->NumeroCollegamenti,CollegamentoCorrente,COLLEGAMENTO){
         ...
         WORD NextCosto = pNodoScanned->Costo1 + CollegamentoCorrente->Costo;
         ...
         if(NextNodo.Costo1 > NextCosto){
            ...
            NextNodo.Nodo1  = pNodoScanned->Iid();

            #ifndef DBG7
            assert2(NextNodo.Stato != SCANNED,VRS(pNodoScanned->Id)+VRS(NextNodo.Id));
            #endif

            if (NextNodo.Stato == LABELED) {
               //TRACESTRING("HEAP/SCAN Heap.DecrementKey(&NextNodo);");
               Heap.DecrementKey(&NextNodo);
            } else {
               //TRACESTRING("HEAP/SCAN Heap.Insert(&NextNodo);");
               NextNodo.Stato  = LABELED;
               Heap.Insert(&NextNodo);
            } /* endif */
            #ifdef DBG4
            //NextNodo.TraceId(" LABEL Nodo Costo = "+STRINGA(NextNodo.Costo1) +" Ramo Verso "+STRINGA(pNodoScanned->Id)+ " Costo "+STRINGA(CollegamentoCorrente->Costo) );
            #endif
            #ifdef DBG2
            CountLabeled ++;
            #endif
...
void __WIN_FASTCALL PATH_CAMBI::OrariCompatibili(){
   #undef TRCRTN
...
//  PATH_ILLEGALE::Set()
//----------------------------------------------------------------------------
void __WIN_FASTCALL PATH_ILLEGALE::Set(const class  PATH_CAMBI & Da){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
void __WIN_FASTCALL PATH_CAMBI::CompatibilePrecedenti(){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
void __WIN_FASTCALL PATH_CAMBI::DichiaraIncompatibile(BYTE PosNodoDiCambio) {
   #undef TRCRTN
...
//----------------------------------------------------------------------------
void __WIN_FASTCALL PATH_CAMBI::CalcolaCostoDiramazione(){
   #undef TRCRTN
...
void __WIN_FASTCALL PATH_CAMBI::CompletaPercorso(){
   #undef TRCRTN
   ...
   while( Ide->Id != Destinazione && NumNodi <= MAX_TRATTE ){
      ...
      if(!Nodo->Esteso){ // Nodo non ancora esteso
         TRACESTRING("Prima di Nodo->Estendi();");
         Nodo->Estendi();
         TRACESTRING("Dopo di Nodo->Estendi();");
      }
...
int  __WIN_FASTCALL IdentificaFasciaDiCollegamento(WORD Ora);
void Fasce_Init(){
   #undef TRCRTN
