...
void DATI_ORARIO_FS::Free(){
   ...
   while(LimiteNodi > BaseNodi) {
      ...
      if (NODO_CAMBIO::TotNodiDiCambio > BaseNodi + 1) {
         ...
         // begin EMS003 Win sostituisco SCAN_NUM_WIN a SCAN_NUM
         // Pv2 diventa un puntatore
         SCAN_NUM_WIN(Nc.Collegamenti(),Nc.NumeroCollegamenti,Pv2,COLLEGAMENTO) {
            NODO_CAMBIO & Altro = NodiDiCambio[Pv2->Id];
            BOOL Done = FALSE;
            // begin EMS003 Win sostituisco SCAN_NUM_WIN a SCAN_NUM
            // Pv3 diventa un puntatore
            SCAN_NUM_WIN(Altro.Collegamenti(),Altro.NumeroCollegamenti,Pv3,COLLEGAMENTO) {
...
BOOL DATI_ORARIO_FS::DefinisciNodoDiCambio(ID Esterno){
   ...
   if(NODO_CAMBIO::NodiDiCambio){ // Altrimenti e' il caricamento iniziale
      ...
      // begin EMS004 Win porto fuori i refernce oppure non li uso
      // i reference Scl e Scl2 sono stati sostituiti ovunque con gli oggetti referenziati
      // iId_cluster_corrente è stato portato fuori ciclo
      int iId_cluster_corrente;
      for (FClusters.Seek(Esterno);FClusters.RecordCorrente().Stazione == Esterno ;FClusters.Next() ) {
         ...
         iId_cluster_corrente = FClusters.RecordCorrente().IdCluster;
         Collider.Set(iId_cluster_corrente);
         NODO_CAMBIO::StzClusters[IdxNuovo2] = FClusters.RecordCorrente();
         NODO_CAMBIO::StzClusters[IdxNuovo2].Stazione =
               Interno(NODO_CAMBIO::StzClusters[IdxNuovo2].Stazione);
         IdxNuovo2++;
         ...
         for (int j  = Last->NumeroClusters ; j > 0; j-- ) {
            if(Collider.Test(Clust->IdCluster)){ // OK Ha lo stesso cluster
               ...
               if(Clust->Gruppi & Clust2->Gruppi){
                  ...
                  if(Coll == NULL){
                     // begin EMS005 Win i postincrementi li metto fuori
                     Coll = Last->Collegamenti(Last->NumeroCollegamenti);
                     Last->NumeroCollegamenti ++;
                     Coll2= Nuovo.Collegamenti(Nuovo.NumeroCollegamenti);
                     Nuovo.NumeroCollegamenti ++;
                     // end EMS005 Win
...
//----------------------------------------------------------------------------
BOOL __WIN_FASTCALL CLU_BUFR::RidottoTempoCoinc(BYTE P_Nodo){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
void __WIN_FASTCALL CLU_BUFR::DeterminaServizi(BYTE P_Da, BYTE P_A, int GG, MM_INFO & Out){
   #undef TRCRTN
