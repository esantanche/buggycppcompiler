...
//----------------------------------------------------------------------------
BOOL __WIN_FASTCALL COLLEGAMENTO_DIRETTO::InitForCache(void *, void *){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
void __WIN_FASTCALL COLLEGAMENTO_DIRETTO::RemoveFromCache(void *){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
COMB __WIN_FASTCALL COLLEGAMENTO_DIRETTO::PosizionaPrimo()     { // Per prima tratta o usato internamente dagli altri Posiziona
   #undef TRCRTN
...
//----------------------------------------------------------------------------
COMB __WIN_FASTCALL COLLEGAMENTO_DIRETTO::PosizionaIntermedio(){ // Per tratte intermedie
   #undef TRCRTN
...
//----------------------------------------------------------------------------
COMB __WIN_FASTCALL COLLEGAMENTO_DIRETTO::PosizionaUltimo()    { // Per ultima tratta
   #undef TRCRTN
...
//----------------------------------------------------------------------------
int  __WIN_FASTCALL COLLEGAMENTO_DIRETTO::IdentificaAlternative(BOOL Last) {
   #undef TRCRTN
...
// AttesaValida
//----------------------------------------------------------------------------
WORD __WIN_FASTCALL AttesaValida( WORD LastOraArrivo, WORD OraPartenza){ // Calcola l' attesa tra due ore (escludendo le ore notturne)
   #undef TRCRTN
...
//----------------------------------------------------------------------------
CLUSTRENO * __WIN_FASTCALL COLLEGAMENTO_DIRETTO::GetTrenoInCluster(){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
INFOSTAZ  * __WIN_FASTCALL COLLEGAMENTO_DIRETTO::GetDatiStazioneInCluster(){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
int __WIN_FASTCALL COLLEGAMENTO_DIRETTO::IdMezv(){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
int __WIN_FASTCALL COLLEGAMENTO_DIRETTO::IdCluster(){
   #undef TRCRTN
...
//----------------------------------------------------------------------------
char  *  __WIN_FASTCALL COLLEGAMENTO_DIRETTO::IdTreno(){
   #undef TRCRTN
...
// IPOTESI::OraPartenza
//----------------------------------------------------------------------------
WORD __WIN_FASTCALL IPOTESI::OraPartenza(BYTE Tratta){
   #undef TRCRTN
...
// IPOTESI::OraArrivo
//----------------------------------------------------------------------------
WORD __WIN_FASTCALL IPOTESI::OraArrivo(BYTE Tratta){
   #undef TRCRTN
...
// IPOTESI::Clust
//----------------------------------------------------------------------------
T_CLUST & __WIN_FASTCALL IPOTESI::Clust(BYTE i){
   #undef TRCRTN
...
void IPOTESI::Combina(){
   ...
   for (int Ico = 0; Ico < NumTratte ; Ico ++) {
      ...
      // EMS003 Win aggiungo il parametro FALSE in fondo
      if(Cd.MultiStaz)Cd.MS.DatiCluster->Dump( Esterno(Cd.Key.Partenza), Esterno(Cd.Key.Arrivo), FALSE);
      for (int i = 0; i < Cd.NumClusters ; i++ ) {
         // EMS003 Win aggiungo il parametro FALSE in fondo
         Cd.Clusters[i].DatiCluster->Dump( Esterno(Cd.Key.Partenza), Esterno(Cd.Key.Arrivo), FALSE);
      } /* endfor */
   } /* endfor */
   #endif
...
//<<< if  Coll.UsareMultistazione
         ...
         // begin EMS004 Win espansione di chiamate

         short sTreno_corrente = Coll.TrenoCorrente;
         T_TRENO & Treno = Coll.Treni[sTreno_corrente];
         BYTE bPCluster = Treno.P_Cluster;
         T_CLUST & Clust = Coll.Clusters[bPCluster];
         ...
         // begin EMS005
         //CLU_BUFR & Clu = * (Clust.DatiCluster) ;  // Dati del cluster
         CLU_BUFR & Clu = * (PClust->DatiCluster) ;  // Dati del cluster
         // END EMS005
         //TRACEPOINTER("Clust.DatiCluster 2", Clust.DatiCluster);
         PClu = &Clu;
         Clu.FindTreno(Treno.P_Treno);
         // TRACESTRING(VRS(Clust.IdCluster) + VRS(Clust.Concorde) + VRS(Clu.Treno)
         // + VRS(Clu.ProgTreno()) +VRS(Clu.Treno->IdMezv) +
         //VRS(Esterno(Coll.Key.Partenza)) + VRS(Esterno(Coll.Key.Arrivo  )) +
         // VRS(Clust.PNodoIn )+ VRS(Clust.PNodoOut) );
...
// EMS006 VA struttura RICH portata fuori della funzione CLU_BUFR::Dump
...
void CLU_BUFR::Dump(ID Da, ID A , BOOL Force){   // Da ed A sono Id Esterni
   ...
   // EMS006 VA struttura RICH portata fuori
