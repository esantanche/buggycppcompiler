...
// EMS001 Win #pragma option -O2 -O-c -O-z      // Mi serve il massimo delle performance (NB: Oz rallenta)
...
BASE_ALLOCATOR::BLOCCO * MMB_FASTCALL BASE_ALLOCATOR::AllocaBLOCCO(){
   ...
   // EMS003 Win azzeramento dopo malloc
   memset((char *) Wrk, 0L,  DimBlocco + sizeof(BLOCCO));
   MEMIGNORE(Wrk);
   Wrk->Next              = NULL;
   Wrk->Dati              = (void*)(Wrk+1);
...
Allocator(DimElemento+sizeof(ESTENSIONE),NumDatiAllocIniziali)
{
   ...
   // begin EMS004 Win azzeramento array di oggetti di classe CELLA
...
void   BASE_HASH::Istogram(const STRINGA& Msg)const{
   ...
   SCAN_NUM_WIN(Celle,NumCelle,Cella,CELLA){
      DWORD Count = 0;
...
// begin EMS006 tolgo inline
int HEAP<TEST,2>::Confronta(TEST * a, TEST *b){return a->Key - b->Key;};
int HEAP<TEST,3>::Confronta(TEST * a, TEST *b){return a->Key - b->Key;};
int HEAP<TEST,4>::Confronta(TEST * a, TEST *b){return a->Key - b->Key;};
// end EMS006
#endif
...
// EMS001 Win #pragma option -Od   // Su questo modulo l' ottimizzatore crea problemi
// ---------------------------------------------------------------------
// BASE_HASH::Hash"
// ---------------------------------------------------------------------
