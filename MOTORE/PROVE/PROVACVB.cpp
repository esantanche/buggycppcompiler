...
void main(int argc,char * argv[]) {
	...
	//FIn = stdin;
	//FOut = stdout;
	FIn  = fopen("SUITE2.IN","rt");
	FOut = fopen("SUITE2.OUT","wt");
	TRACEREGISTER2(NULL,"","PROVACVB.TRC");

	FILE_RW Out("\\D\\MOTORE\\PROVE\\PROVAcvb.OUT");
	Out.SetSize(0);
   ...
   Bprintf("Inizio PROVACVB.CPP tracelevel= %i\n",trchse);
      ...
      percSol =(MM_ELENCO_PERCORSO_E_SOLUZIONI*) NULL;
		...
		} else
		{
			ORD_FORALL(PercSol,is){
				...
				Bprintf("%s Km: FS %i Concessi %i Concessi2 %i Mare %i Tirrenia %i\n",
