...
int  main(int argc,char * argv[]) {
   ...
   BOOL SOLO_SINTAX_CHECK = argc > 1 && !stricmp(argv[1],"/s");
...
//<<<          for  int i = 0;i < Sol->NumeroTratte ;i++    
               ...
               // Trace dei km parziali
               if (fKmParziali && Sol->NumeroTratte >1){
                  for (int Nt = 0; Nt < Sol->NumeroTratte ; Nt ++) {
                     MM_SOLUZIONE::TRATTA_TRENO & Tratta = (MM_SOLUZIONE::TRATTA_TRENO &)Sol->Tratte[Nt];
                     if (Tratta.IdMezzoVirtuale == 0)continue; // Tratta urbana
                     ERRSTRING("Tariffazione parziale Tratta Nø "+STRINGA(Nt));
                     DATI_TARIFFAZIONE_2 Dt = Sol->KmParziali(Nt,Nt,&Perc);
                  } /* endfor */
               } /* endif */
//<<<       ORD_FORALL Sols,i2  
            };
...
BOOL chiedi(ID& Da ,ID& A, MM_RETE_FS & Rete) {
   ...
   fprintf(FOut,"\n");
   do {
      Trc = Print || fTratta || fPeriodicita || !Batch || fDettagliTariffe || fKmParziali ;
      if(!Batch){
         ...
         puts("   Km parziali Toggle    : Abilita / Disabilita il TRACE dei Km parziali su ogni tratta");
      ...
      if (uno[0]== 'K'||uno[0]== 'k') fKmParziali  = ! fKmParziali ;
      ...
      if (uno[0]=='œ') {
         ...
         if(Vals.Dim() >= 2){
            ...
            if(!(Vals[0] == STRINGA(StAll->RecordCorrente().NomeStazione)(0,Vals[0].Dim()-1))){
               ...
               fprintf(stderr,"Non identificata stazione '%s'\n",(CPSZ)Vals[0]);
               Id1 = 0;
            }
