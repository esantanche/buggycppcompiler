...
STRINGA operator+(const STRINGA & a, const char * b)
{
   if(&a == NULL){
      ...
      return STRINGA(a.Dati,a.Len,b,strlen(b));
   }
}
...
STRINGA operator+(const char * b,const STRINGA & a)
{
   if(&a == NULL){
      ...
      return STRINGA(b,strlen(b),a.Dati,a.Len);
   }
}
...
STRINGA operator+ (const STRINGA & a, const char b)
{
   ...
   return STRINGA(a.Dati,aLung,Strg,1);
}
...
STRINGA STRINGA::operator()(int Primo, int Ultimo)const
{
   if(this != NULL && Len >0 && Ultimo >= 0 && Primo < Len && Ultimo >= Primo)
   {
      ...
      if(Primo < 0)Primo = 0;
      return STRINGA(Dati + Primo,Ultimo - Primo + 1);
   } else {
      return STRINGA();
   }
}
