...
static HFILE FileHandle=NULL;
static HFILE  Lpt1FileHandle=NULL;
static BOOL   Lpt1Enabled=FALSE;
static USHORT Lpt1Tid;
static HFILE  LogFileHandle=NULL;
...
void cdecl PrtParms(CPSZ R,int argc,PSZ  argv[]){
   ...
   if (!strcmp(Trch_Programma,"PGMNONSO") || Trch_Programma[0] == 0) {
      if(argc > 0)strcpy(Trch_Programma,argv[0]);
   } /* endif */
   int i;
   PrtText (R,"=================================================================");
   ...
   for (i=0;i<argc;i++)PrtText(R,argv[i]);
   char CD[250];
   ...
   DosQueryCurrentDir ( DriveNumber,  DirPath+2, &DirPathLen);
