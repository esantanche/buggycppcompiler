...
//<<< if OffSet != Header.TotalSize  
   ...
   if(ReBuild){
      ...
      ORD_FORALL(FCollegamenti,i1){ // Il file contiene i soli collegamenti tra nodi di cambio
         COLLEGAMENTO_F & Cst = FCollegamenti[i1];
         ...
         COLLEGAMENTO & Coll = NODO_CAMBIO::AllCollegamenti[Count];
         ...
         #ifdef DBGLOAD
         TRACESTRING(VRS(Count)+VRS(Cst.StazionePartenza)+VRS(Cst.StazioneArrivo)+VRS(Coll.Km)+VRS(Coll.PesoTratta)+VRS(Coll.Penalita)+VRS(Coll.OrariPartenza)+VRS(Coll.OrariArrivo));
         #endif
...
short int CLU_BUFR::KmMediDaId(ID Id){
   ...
   SCAN_NUM(Nodi,Dat->TotStazioni,Nod,CLUSSTAZ){
...
int __fastcall CLU_BUFR::PosNodoInCluster(ID Id){  // Id e' l' ID Esterno del nodo
   #undef TRCRTN
...
//----------------------------------------------------------------------------
void __fastcall CLU_BUFR::FreeCluster(ID IdCluster , BYTE Concorde){
   #undef TRCRTN
...
// CLU_BUFR::GetCluster
//----------------------------------------------------------------------------
CLU_BUFR * __fastcall CLU_BUFR::GetCluster(ID IdCluster , BYTE Concorde, BYTE PuoiEseguireIO ){
   #undef TRCRTN
