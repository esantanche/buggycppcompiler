...
//----------------------------------------------------------------------------
void __fastcall F_ECCZ_NORM::DeterminaIdx(int Tipo, int & Minimo, int & Massimo){ // Determina il minimo e massimo di scansione per tipo
   #undef TRCRTN
...
DIST __fastcall GRAFO::DistanzaTra(ID Da, ID A){
   #undef TRCRTN
...
BOOL GRAFO::Risolvi(ID Da, ID A, int MaxDist ){
   ...
   if(++ ProgressivoCorrente  == 0){  // Reset di tutti i progressivi (la prima volta )
      ...
      SCAN_NUM(StazioniGrafo,TotStazioniGrafo,Staz,STAZ_FS) {
         Staz.Progressivo = 0;
      } ENDSCAN ;
   };
...
BOOL GRAFO::IdentificaRamo(ID Staz1,ID Staz2, ID & IdRamo,int & Km){
   ...
   if (sDa.IsNodo()) {
      if (sA.IsNodo()) {
         SCAN_NUM(sA.PrimiVicini(),sA.NumeroPrimiVicini,PrimoVicinoCorrente,STAZ_FS::PRIMO_V){
...
int STAZ_FS::HaPolimetrica(ID IdPolimetrica){
   ...
   #undef TRCRTN
   SCAN_NUM(Polimetriche(),NumeroPolimetriche,Pol,P_POLI){
...
BOOL POLIMETRICA::Contiene(ID Staz){
   ...
   SCAN_NUM(S1.Polimetriche(),S1.NumeroPolimetriche,Pol1,STAZ_FS::P_POLI){
...
DIST POLIMETRICA::DistanzaTra(ID From,ID To,BYTE & Verso){ // Torna BigDIST se non appartengono entrambi ala polimetrica
   ...
   SCAN_NUM(S2.Polimetriche(),S2.NumeroPolimetriche,Pol2,STAZ_FS::P_POLI){
...
DATI_TARIFFAZIONE & PERCORSO_GRAFO::LunghezzaTariffabile(BYTE Modalita){
   ...
   #ifdef DBG2P
   Trace("ÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛPercorso grafo da valorizzare");
   #endif

	// EMS ?C?C
	TRACESTRING("Entro in LunghezzaTariffabile");
            ...
            if(Corrente.IsNodo()){
               ...
               if(Last != 0){
                  ...
                  if(&Pv != NULL && Pv.Instradamento){ // Ad evitare abend
                     ...
                     SCAN_NUM(Ramo.StazioniDelRamo(),Ramo.NumStazioni,Stazione,RAMO::STAZIONE_R){
                        #ifdef ENABLE_INNER // Abilita il trace sugli inner LOOPS
                        PROFILER::Conta(29);
                        #endif
                        if(Stazione.Instrad){
...
//<<<          if Last != 0
					...
					if (j == MaxVincolo && j < MaxNodi &&  IsNodo(Percorso[j+1])) {
						...
						if(&Pv != NULL && Pv.Instradamento){ // Ad evitare abend
							...
							RAMO & Ramo = Grafo.Rami[Pv.IdRamo];
							// EMS
							TRACEINT("++ Ramo.NumStazioni ", Ramo.NumStazioni);
							TRACEINT("++ Pv.IdRamo ", Pv.IdRamo);
							TRACEINT("++ Ramo.IdRamo ", Ramo.IdRamo);
...
//<<< if  Ok   
   ...
   // risultato e' corretto.
   {
      ...
      SCAN_NUM(Conc,Grafo[Origine].NumeroConcesse,DatiConcessi,STAZ_FS::D_CONCESSE){
   ...
   }
   {
      ...
      SCAN_NUM(Conc,Grafo[Destinazione].NumeroConcesse,DatiConcessi,STAZ_FS::D_CONCESSE){
         if(DatiConcessi.SocietaConcessa == SocietaConcessa){
