...
BOOL STAZIONI::R_STRU::InitForCache(class STAZIONI::R_STRU * Key, void *){
   ...
   // TRACEVLONG(Key->IdStazione);
   // TRACEVPOINTER(this);
   // Accedo al file
   if(FStazioni->Posiziona(Key->IdStazione)){
      memmove(this,&FStazioni->RecordCorrente(), sizeof(STAZIONI::R_STRU));
      assert(Id == IdStazione || IdStazione == 0);
      IdStazione = Id;
      return TRUE;
   } else {
      // TRACEINT("Non trovato su file, ID = ",Key->IdStazione);
      // TRACEVPOINTER(Key);
      ZeroFill(THIS);
      IdStazione = Id;
      return FALSE;
