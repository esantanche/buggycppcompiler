...
// EMS001 Win elimino ottimizzazioni #pragma option -Od
//----------------------------------------------------------------------------
/*
...
// EMS004 VA devo portare fuori questa struttura
struct TESTNOME {
      DWORD Ccr;
      char Nome[20];
};
...
int main(int argc,char *argv[]){
   ...
   // EMS005 Win SetStatoProcesso(PGM, SP_PARTITO);
   if(trchse > 2)trchse = 2; // Per evitare trace abissali involontari
   // EMS005 Win SetPriorita(); // Imposta la priorita'
   ...
   // EMS004 VA devo portare fuori la struttura TESTNOME
   ...
   for (int i = 0 ;i < Nr ; i++ ) { // Scansione Localita'
      ...
      if(Loc[i].TestRecord != 'A'){
         ...
         LastTreno = (const char *) Loc.RecordCorrente().IdentTreno;  // EMS006 VA (const char *)
      }
      Ok ++;
...
//<<< for  int i = 0 ;i < Nr ; i++     // Scansione Localita'
   ...
   TRACESTRING("Uscito dal ciclo di scansione località 304");
      ...
      ORD_FORALL(MezziVirtuali,Tv){
         ...
         for (int i = 0; i < TrenoVirtuale.NumMezziComponenti; i++ ){
            ...
            if (i < (TrenoVirtuale.NumMezziComponenti-1) && Ccr_Id.Id(Mc.StazioneDiCambio.Cod()) == 0 ) {
               STRINGA T1((const char *) TrenoVirtuale.MezziComponenti[i].IdentTreno); // EMS006 VA (const char *)
               STRINGA T2((const char *) TrenoVirtuale.MezziComponenti[i+1].IdentTreno); // EMS006 VA (const char *)
               ...
               case 5:
                  {
                     ...
                     if((DopoSkipIniziale || Lo.Codice.Cod() == LastCambio) &&
                        ...
                        if( OraP >= LastPartenza){
                           ...
                           STRINGA T1((const char *) Mc.IdentTreno);  // EMS006 VA (const char *)
