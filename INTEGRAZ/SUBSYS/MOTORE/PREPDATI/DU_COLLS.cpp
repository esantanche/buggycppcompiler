...
// Qui va la definizione della struttura
   ID   StazionePartenza;
   ID   StazioneArrivo  ; // Si ha sempre StazioneArrivo > StazionePartenza
   WORD Count           ; // Numero treni che realizzano il collegamento
   ...
   WORD OrariArrivo     ; // 10 BIT. I BITS non rappresentano l' ora di partenza ma
   ...
   WORD OrariArrivo2    ; // Idem , ma considerando i tempi massimi di coincidenza
};
#define NOME "COLLSTAZ"
//----------------------------------------------------------------------------
...
int main(int argc,char *argv[]){
   ...
   if(argc < 2){
      printf("Utilizzo: DU_COLLS Stazione [... Stazione]");
      printf(" DU_COLLS 0: Dump dei collegamenti di TUTTE le stazioni di cambio con ALTRE stazioni di cambio ");
      return 99;
   };
...
// Qui va la definizione dei campi di stampa (la dim deve comprendere lo spazio verso il prossimo campo)
   ...
   MK1( OrariVeriArrivo  , "6","X","Ore","VereA")
...
//----------------------------------------------------------------------------
   ...
   MK2( OrariVeriArrivo  , "6","X","Ore","VereA")
