...
// E queste sono variabili di lavoro
   ...
   FORALL(Arr,i2){
      ...
      if ( Arr2.TrenoInizioServizioSeMV != NESSUN_TRENO ) {
         ...
         for( NOTA_SERVIZI  * Wrk = Hash.Cerca( &WrkNota, KLen); Wrk; Wrk = Hash.CercaNext()) {
            #ifdef DBG3
            TRACESTRING( "In Hash: " VRS(WrkNota.IdentTreno) + VRS(WrkNota.IdNota));
         ...
         if(Wrk == NULL){ // Non trovato
            ...
            for( Wrk = Hash2.Cerca( &WrkNota, KLen); Wrk; Wrk = Hash2.CercaNext()) {
               #ifdef DBG3
               TRACESTRING("In Hash2: " VRS(Arr2.StazioneFineServizio) + VRS(Wrk->StazioneInizioServizio) );
               #endif
