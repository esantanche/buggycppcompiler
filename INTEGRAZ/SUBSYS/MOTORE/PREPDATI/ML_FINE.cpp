...
// EMS 002 #define LIVELLO_DI_TRACE_DEL_PROGRAMMA  4 // Mettere ad 1 per attivare il debug
...
// EMS002
#define LIVELLO_DI_TRACE_DEL_PROGRAMMA  1 // Mettere ad 1 per attivare il debug
...
int WORK::Body(){
   ...
   bSuccess = Validita.Posiziona(sizeof( VALIDITA_ORARIO )); // EMS004
   if (!bSuccess) TRACESTRING("Errore in Validita.Posiziona");   // EMS004 Win
   bSuccess = Validita.Scrivi(VRB(Header)); // EMS004 Contemporaneamente cambia data ed ora di VALIDITA.DB
   if (!bSuccess) TRACESTRING("Errore in Validita.Scrivi");   // EMS004 Win
...
int WORK::CaricaStazioniDelcluster( CLUSTER_MV & Clu){
   ...
   for (ClusStaz.Seek(Clu.Id);
      ...
      TRACEINT("ASSEGNO MediaKm Cs.DistanzaMediana",Cs.DistanzaMediana);
      Clusta_Ext_Rec2[Cs.Prog].MediaKm              =   ChkS(Cs.DistanzaMediana,13);
      TRACEINT("ASSEGNO MediaKm Clusta_Ext_Rec2[Cs.Prog].MediaKm ",Clusta_Ext_Rec2[Cs.Prog].MediaKm );
...
int  WORK::CaricaDatiDiFermataMV(CLUSTER_MV & Clu, int ProgressivoPeriodicita ){
   ...
   while (
      ...
      TRACEINT("Id in ciclo while",TabFv2.RecordCorrente().Id);
      if(Idx != 0xff){ // Controllo che non sia un transito DA IGNORARE
         ...
         TRACEINT("KmCorretti",KmCorretti);
         TRACEINT("Clusta_Ext_Rec2[Idx].MediaKm",Clusta_Ext_Rec2[Idx].MediaKm);
         ...
         TRACEINT("DELTA DELTA DELTA",Delta);
         if(abs(Delta) > 255){
            STRINGA Msg;
